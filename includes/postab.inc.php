...
//tabela
$sql42 = "SELECT data FROM teste_valores WHERE uid ='". $uid ."'";
$qr = mysqli_query($conn, $sql42);

$sqlQuery =   "SELECT pitch0, roll0, pitch1, roll1, pitch2, roll2, pitch3, roll3, pitch4, roll4  FROM teste_valores WHERE  pitch0 <= '$mediapitch0' + '$desviopitch0' AND pitch0 >= '$mediapitch0' - '$desviopitch0' AND roll0 <= '$mediaroll0' + '$desvioroll0' AND roll0 >= '$mediaroll0' - '$desvioroll0' AND pitch1 <= '$mediapitch1' + '$desviopitch1' AND pitch1 >= '$mediapitch1' - '$desviopitch1' AND roll1 <= '$mediaroll1' + '$desvioroll1' AND roll1 >= '$mediaroll1' - '$desvioroll1' AND pitch2 <= '$mediapitch2' + '$desviopitch2' AND pitch2 >= '$mediapitch2' - '$desviopitch2' AND roll2 <= '$mediaroll2' + '$desvioroll2' AND roll2 >= '$mediaroll2' - '$desvioroll2' AND pitch3 <= '$mediapitch3' + '$desviopitch3' AND pitch3 >= '$mediapitch3' - '$desviopitch3' AND roll3 <= '$mediaroll3' + '$desvioroll3' AND roll3 >= '$mediaroll3' - '$desvioroll3' AND pitch4 <= '$mediapitch4' + '$desviopitch4' AND pitch4 >= '$mediapitch4' - '$desviopitch4' AND roll4 <= '$mediaroll4' + '$desvioroll4' AND roll4 >= '$mediaroll4' - '$desvioroll4' AND uid ='".$uid."'";
...
echo"<td style='color:#007aa8'>Boa Postura";
}
else{
  echo"<td style='color:#ff3345'>Má Postura";
}
